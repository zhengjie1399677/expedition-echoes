...
const KEY = 'expedition-echoes.save.v11';
...
type StoredGame = Omit<GameState, 'version' | 'roster' | 'inventory' | 'materials' | 'hasAcceptedMission' | 'day' | 'missionAcceptedToday' | 'food' | 'hunger' | 'giftsGivenToday'> & { version: number; roster: StoredHero[]; inventory?: Record<string, number>; materials?: Record<string, number>; hasAcceptedMission?: boolean; day?: number; missionAcceptedToday?: boolean; food?: number; hunger?: number; giftsGivenToday?: Record<string, number> };
export function loadGame(): GameState | null { try {
  LEGACY_KEYS.forEach((key) => localStorage.removeItem(key));
  const raw = localStorage.getItem(KEY) ?? localStorage.getItem(V10_KEY) ?? localStorage.getItem(V9_KEY) ?? localStorage.getItem(V8_KEY) ?? localStorage.getItem(V7_KEY) ?? localStorage.getItem(V6_KEY) ?? localStorage.getItem(V5_KEY); if (!raw) return null;
  const parsed = JSON.parse(raw) as StoredGame;
  if (parsed.version < 5 || parsed.version > 11) return null;
  ...
  if (parsed.version < 11) { localStorage.removeItem(V10_KEY); localStorage.removeItem(V9_KEY); localStorage.removeItem(V8_KEY); localStorage.removeItem(V7_KEY); localStorage.removeItem(V6_KEY); localStorage.removeItem(V5_KEY); }
  return state;
} catch { return null; } }
...
export function clearGame(): void { try { localStorage.removeItem(KEY); localStorage.removeItem(V10_KEY); localStorage.removeItem(V9_KEY); localStorage.removeItem(V8_KEY); localStorage.removeItem(V7_KEY); localStorage.removeItem(V6_KEY); localStorage.removeItem(V5_KEY); LEGACY_KEYS.forEach((key) => localStorage.removeItem(key)); } catch { /* ignore */ } }
