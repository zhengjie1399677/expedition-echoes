...
const KEY = 'expedition-echoes.save.v1';
export function loadGame(): GameState | null { try { const raw = localStorage.getItem(KEY); if (!raw) return null; const state = JSON.parse(raw) as GameState; return state.version === 1 ? state : null; } catch { return null; } }
